...
public partial class ReportesViewModel(IPedidoRepository repository):ObservableObject
{
 ...
 [RelayCommand]public async Task ConsultarAsync(){if(FechaInicio is null||FechaFin is null){Mensaje="Selecciona ambas fechas.";EsError=true;return;}if(FechaInicio>FechaFin){Mensaje="La fecha inicial no puede ser posterior a la fecha final.";EsError=true;return;}EstaCargando=true;Mensaje=null;try{var a=await repository.ReportarAsync(FechaInicio.Value.Date,FechaFin.Value.Date);Lineas.Clear();foreach(var x in a)Lineas.Add(x);Total=Lineas.Sum(x=>x.Subtotal);Mensaje=Lineas.Count==0?"No existen detalles de pedidos en el intervalo.":$"{Lineas.Count} detalle(s) encontrado(s).";EsError=false;}catch(Exception e){Mensaje=$"No se pudo consultar el reporte: {e.Message}";EsError=true;}finally{EstaCargando=false;}}
}
